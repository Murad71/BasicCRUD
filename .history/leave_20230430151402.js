...
for(let i=0; i<editListItems.length; i++)
{
    ...
    button.addEventListener('click', function(event){
        ...
        if(alert)
        {
            ...
            let row= buttonClicked.closest('tr');
            let rowIndex= row.rowIndex;
            document.getElementById('name').value = rowIndex.name;
            document.getElementById('startdate').value = rowIndex.startdate;
            document.getElementById('days').value = rowIndex.days;
            document.getElementById('leave').value = rowIndex.leaves; 
        }
        else return;
    })
}
