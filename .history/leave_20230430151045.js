...
for(let i=0; i<editListItems.length; i++)
{
    ...
    button.addEventListener('click', function(event){
        ...
        if(alert)
        {
            document.getElementById('name').value = editListItems[i].name;
            document.getElementById('startdate').value = editListItems[i].startdate;
            document.getElementById('days').value = editListItems[i].days;
            document.getElementById('leave').value = editListItems[i].leaves; 
        }
        else return;
    })
}
