...
for(let i=0; i<editListItems.length; i++)
{
    ...
    button.addEventListener('click', function(event){
        ...
        if(alert)
        {
            ...
            let name = tableRow.cells[0].textContent;
            let startDate = tableRow.cells[1].textContent;
            let days = tableRow.cells[2].textContent;
            let leaveType = tableRow.cells[3].textContent;
            let photo = tableRow.cells[4].querySelector('photo').src;
